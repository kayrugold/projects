...
function startGCD() {
    ...
    log('--- Starting Karatsuba-Euclidean GCD (Main Thread) ---');
    ...
    if (!n1_a || !n1_b || !n1_c || !n2_a || !n2_b || !n2_c) {
        log("Error: All base, exponent, and addend inputs must be provided.");
        setStatus('Error: Missing input.');
        stopCalculation();
        return;
    }
    
    // Check exponents are integers
    if (BigInt(n1_b) < 0n || BigInt(n2_b) < 0n || n1_b.includes('.') || n2_b.includes('.')) {
        log("Error: Exponents must be non-negative integers.");
        setStatus('Error: Invalid Exponent.');
        stopCalculation();
        return;
    }
    ...
    // Load the worker from its external file
    ...
    gcdWorker.onmessage = (e) => {
        if (e.data.type === 'progress') {
            ...
            log(`\nFast Euclidean GCD complete in ${steps} steps.`);
